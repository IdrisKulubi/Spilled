...
export const authUtils = {
    ...
    try {
      ...
      if (data.url) {
        ...
        if (result.type === "success") {
          console.log(
            "✅ [OAuth] OAuth completed successfully, processing callback..."
          );
          ...
          if (result.url) {
            ...
            try {
              ...
              if (accessToken && refreshToken) {
                console.log(
                  "🔄 [OAuth] Setting session with tokens from callback URL"
                );
                ...
                if (error) {
                  ...
                  console.log(
                    "ℹ️ [OAuth] Profile creation delegated to auth state change handler"
                  );
              ...
              console.log(
                "⚠️ [OAuth] Session processing failed:",
                exchangeError
              );
            }
          }
          ...
          while (attempts < maxAttempts && !user) {
            attempts++;
            console.log(
              `🔄 [OAuth] Attempt ${attempts} to get user session...`
            );
          ...
          if (!user) {
            console.error(
              "❌ [OAuth] Failed to establish session after",
              maxAttempts,
              "attempts"
            );
            return {
          ...
          try {
            ...
            console.error(
              "❌ [OAuth] Failed to create user profile:",
              profileError
            );
            return {
              success: false,
  ...
  getCurrentUser: async (): Promise<User | null> => {
    try {
      ...
      if (!profile) {
        console.log(
          "⚠️ [Auth] No profile found in database for user:",
          user.id
        );
        return null;
      }
  ...
  ensureUserProfile: async (
    ...
    try {
      ...
      console.log(
        "📄 [Profile] Existing profile check - found:",
        existing ? "yes" : "no"
      );
    ...
    try {
      ...
      console.log(
        "📝 [Profile] Insert result - user:",
        newUser ? "created" : "failed"
      );
      console.log("📝 [Profile] Insert result - error:", createError);

      if (createError) {
        console.error(
          "❌ [Profile] Failed to create user profile:",
          createError
        );
        throw new Error(
